...
urlpatterns = [
    ...
    # 后台管理员登录，退出
    path('log_in/', index.log_in, name="myadmin_log_in"),  # 加载登录页面
    path('do_login/', index.dologin, name="myadmin_do_login"),  # 执行登录操作
    path('log_out/', index.log_out, name="myadmin_log_out"),  # 退出登录
    path('verify/', index.verify, name="myadmin_lverify"),  # 验证码

    # 员工信息管理路由
    path("user/<int:pIndex>", user.index, name="myadmin_user_index"),  # 用户展示列表
    ...
    # 店铺信息管理路由
    path("shop/<int:pIndex>", shop.index, name="myadmin_shop_index"),  # 用户展示列表
    ...
    # 菜品类别信息管理路由
    path("category/<int:pIndex>", category.index, name="myadmin_category_index"),  # 用户展示列表
    path("category/load/<int:sid>", category.loadCategory, name="myadmin_category_load"),  # 用户展示列表
    ...
    # 菜品信息管理路由
    path("product/<int:pIndex>", product.index, name="myadmin_product_index"),  # 用户展示列表
    path("product/add", product.add, name="myadmin_product_add"),  # 加载添加表单
    ...
    # 会员信息管理路由
    path("member/<int:pIndex>", member.index, name="myadmin_member_index"),  # 用户展示列表
